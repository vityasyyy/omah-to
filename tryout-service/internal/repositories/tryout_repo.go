...
type TryoutRepo interface {
	...
	GetOngoingAttemptByUserIDTx(c context.Context, tx *sqlx.Tx, userID int) (*models.TryoutAttempt, error) // NEW
}
...
func (r *tryoutRepo) CreateTryoutAttemptTx(c context.Context, tx *sqlx.Tx, attempt *models.TryoutAttempt) error {
	...
	if err != nil {
		logger.LogErrorCtx(c, err, "Failed to insert time limits for tryout attempt", map[string]any{"attempt_id": attempt.TryoutAttemptID})
		return err
	}

	logger.LogDebugCtx(c, "Tryout attempt created successfully", map[string]any{"attempt_id": attempt.TryoutAttemptID})
...
func (r *tryoutRepo) GetTryoutAttemptByUserIDTx(c context.Context, tx *sqlx.Tx, userID int) (string, error) {
	...
	if err != nil {
		logger.LogErrorCtx(c, err, "Failed to get ongoing tryout attempt", map[string]any{"user_id": userID})
		return "", err
	}
	return status, nil
}
...
func (r *tryoutRepo) SaveAnswersTx(c context.Context, tx *sqlx.Tx, answers []models.UserAnswer) error {
	...
	if err != nil {
		logger.LogErrorCtx(c, err, "Failed to save user answers", map[string]any{"answers_count": len(answers)})
		return err
	}

	logger.LogDebugCtx(c, "User answers saved/updated", map[string]any{"answers_count": len(answers)})
	return nil
}
...
func (r *tryoutRepo) ProgressTryoutTx(c context.Context, tx *sqlx.Tx, attemptID int, subtest string) (string, error) {
	...
	if err != nil {
		logger.LogErrorCtx(c, err, "Failed to update tryout attempt", map[string]any{"attempt_id": attemptID, "subtest": subtest})
		return "", err
	}

	logger.LogDebugCtx(c, "Tryout attempt progressed", map[string]any{
...
func (r *tryoutRepo) EndTryOutTx(c context.Context, tx *sqlx.Tx, attemptID int) error {
	...
	if err != nil {
		logger.LogErrorCtx(c, err, "Failed to end tryout attempt", map[string]any{
			"attempt_id": attemptID,
		})
		return err
	}
	logger.LogDebugCtx(c, "Tryout attempt ended", map[string]any{
		"attempt_id": attemptID,
	})
...
func (r *tryoutRepo) GetTryoutAttemptTx(c context.Context, tx *sqlx.Tx, attemptID int) (*models.TryoutAttempt, error) {
	...
	logger.LogDebugCtx(c, "Tryout attempt retrieved", map[string]any{
		"attempt_id": attemptID,
	})
	return &attempt, nil
}
...
func (r *tryoutRepo) GetAnswerFromCurrentAttemptAndSubtestTx(c context.Context, tx *sqlx.Tx, attemptID int, subtest string) ([]models.UserAnswer, error) {
	...
	if err != nil {
		logger.LogErrorCtx(c, err, "Failed to get user answers from attempt and subtest", map[string]any{
			"attempt_id": attemptID,
			"subtest":    subtest,
		})
		return nil, err
	...
	logger.LogDebugCtx(c, "User answers retrieved", map[string]any{
		"attempt_id": attemptID,
		"subtest":    subtest,
	})
	return answers, nil
...
func (r *tryoutRepo) GetSubtestTimeTx(c context.Context, tx *sqlx.Tx, attemptID int, subtest string) (time.Time, error) {
	...
	if err != nil {
		logger.LogErrorCtx(c, err, "Failed to get subtest time limit", map[string]any{
			"attempt_id": attemptID,
			"subtest":    subtest,
		})
	...
	logger.LogDebugCtx(c, "Time limit retrieved", map[string]any{
		"attempt_id": attemptID,
		"subtest":    subtest,
...
func (r *tryoutRepo) GetTryoutAttemptByUserIDAndPaket(c context.Context, userID int, paket string) (*models.TryoutAttempt, error) {
	...
	logger.LogDebugCtx(c, "Attempt retrieved", map[string]any{
...
func (r *tryoutRepo) GetTryoutAttempt(c context.Context, attemptID int) (*models.TryoutAttempt, error) {
	...
	if err != nil {
		logger.LogErrorCtx(c, err, "Failed to get tryout attempt", map[string]any{"attempt_id": attemptID})
		return nil, err
	}

	logger.LogDebugCtx(c, "Attempt retrieved", map[string]any{"attempt_id": attemptID})
	return &attempt, nil
}
...
func (r *tryoutRepo) DeleteAttempt(c context.Context, tx *sqlx.Tx, attemptID int) error {
	...
	if err != nil {
		logger.LogErrorCtx(c, err, "Failed to delete attempt", map[string]any{
			"attempt_id": attemptID,
			"error":      err.Error(),
		})
		return err
	...
	logger.LogDebugCtx(c, "Attempt deleted successfully", map[string]any{
		"attempt_id": attemptID,
	})
	return nil
}

func (r *tryoutRepo) GetOngoingAttemptByUserIDTx(c context.Context, tx *sqlx.Tx, userID int) (*models.TryoutAttempt, error) {
	var attempt models.TryoutAttempt
	q := `SELECT * FROM tryout_attempt WHERE user_id = $1 AND status = 'ongoing' LIMIT 1`
	if err := tx.Get(&attempt, q, userID); err != nil {
		logger.LogErrorCtx(c, err, "get ongoing attempt by user", map[string]any{"user_id": userID})
		return nil, err
	}
	return &attempt, nil
}
