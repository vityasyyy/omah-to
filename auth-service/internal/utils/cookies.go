...
func SetAccessAndRefresh(c *gin.Context, accessToken, refreshToken string) error {
	SetCookie(c, "access_token", accessToken, 15*60, "/", "", true, true)        // access token, 15 mins
	SetCookie(c, "refresh_token", refreshToken, 7*24*60*60, "/", "", true, true) // refresh token, 7 days
	return nil
}
